...
from motulator.grid.model import CurrentSourceConverterSystem, GridConverterSystem
...
# %%
class CurrentSourceConverterControlSystem(ControlSystem):
    """
    Grid converter control system.
    ...
    def get_measurement(self, mdl: CurrentSourceConverterSystem) -> Measurements:
        """Get measurements from sensors."""
        i_dc = mdl.converter.meas_dc_current()
        u_c_ab = abc2complex(mdl.ac_filter.meas_capacitor_voltages())
        i_g_ab = abc2complex(mdl.ac_filter.meas_grid_currents())
        return Measurements(u_c_ab, i_g_ab, i_dc)
